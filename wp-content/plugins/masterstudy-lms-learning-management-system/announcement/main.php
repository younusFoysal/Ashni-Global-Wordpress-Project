...
if (!class_exists('STylemixAnnouncements')) {
	class STylemixAnnouncements
	{
		...
		public function scripts($hook)
		{
			if ($hook == 'index.php') {
				$theme_info = time();
				$assets = STM_LMS_URL . 'announcement/assets/';
		...
		public function add_feedback_button() {
			...
			wp_enqueue_style('stm-lms-feedback', STM_LMS_URL . 'announcement/assets/feedback.css');
			wp_enqueue_script('stm-lms-feedback', STM_LMS_URL . 'announcement/assets/feedback.js');

			echo '<a href="#" class="ms-feedback-button">Feedback
				<img src="' . esc_url(STM_LMS_URL . "announcement/assets/icons/feedback.svg") . '">
			</a>';
		}
		...
		public function render_feedback_popup() {
			require_once STM_LMS_PATH . '/announcement/feedback.php';
		}
