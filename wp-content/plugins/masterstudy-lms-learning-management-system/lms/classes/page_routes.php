...
class STM_LMS_Page_Router {
	...
	public function __construct() {
		...
		add_action( 'stm_lms_pages_generated', [ $this, 'reset_config' ] );
		add_action( 'wpcfto_settings_saved', [ $this, 'reset_config' ] );

		add_action( 'save_post', [ $this, 'reset_page_config' ] );

		add_action( 'init', function () {

			$has_config = $this->has_config();

			if ( ! $has_config and !is_admin() ) {
	...
	function modify_switcher( $langs ) {

		if(!defined('ICL_LANGUAGE_CODE')) return $langs;
		...
		foreach ( wp_list_pluck( $this->routes, 'template' ) as $page_index => $page_key ) {
			...
			if ( ! empty( $page['parent'] ) and empty( $page['type'] ) ) {

				if(empty($page['lang'])) $page['lang'] = $this->current_language_code;

				if($page['lang'] === ICL_LANGUAGE_CODE) {
					$current_page = $page;
				}
		...
		if(!empty($current_page) and !empty($current_page['parent']) and !empty($current_page['parent'][0])) {
	...
	function reset_page_config( $post ) {
	...
	function save_config() {
		$week = 7 * 24 * 60 * 60;
		set_transient( self::$pages_transient_name, $this->pages, $week );
		set_transient( self::$pages_config_transient_name, $this->page_config, $week );
		set_transient( self::$pages_routes_transient_name, $this->routes, $week );
	}
	...
	static function fill_subpages_url( &$page_routes, $settings ) {
	...
	function change_page_content( $content ) {
		...
		foreach ( $this->pages as $page_slug => $page_settings ) {
			...
			if ( ! empty( $page_settings['page_id'] ) && is_page( $page_settings['page_id'] ) ) {
				...
				$template = ( ! empty( $page_settings['logged_in'] ) and is_user_logged_in() ) ? $page_settings['logged_in'] : $page_settings['template'];
	...
	function fill_parent_data( &$page ) {
	...
	function generate_routes( &$pages ) {
		...
		foreach ( $pages as $page_key => &$page ) {
			...
			if ( ! empty( $page['sub_pages'] ) ) {
				...
				if ( ! isset( $page['page_id'] ) and empty( $page['post_type'] ) ) {
					$this->routes[] = $page;
				}
	...
	function template( $pages, $lms_path ) {
		...
		if ( ! empty( $pages ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( ! empty( $template_path ) && $lms_path === $template_path ) {
					...
					add_filter( 'template_include', function () use ( $page ) {
	...
	function insert_language_code( &$arr, $code ) {
	...
	static function modify_post_type_query( $query ) {
