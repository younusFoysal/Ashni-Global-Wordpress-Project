...
class STM_LMS_Comments
{
    ...
    static function comments_columns($cols)
    {
            ...
            case 'comment_title' :
                {
                    ...
                    if (in_array(get_post_type($comment_post_id), $lms_types)) {
                        echo sanitize_text_field(get_the_title($comment_post_id));
                    } else { ?>
                        <a href="<?php echo esc_url(get_the_permalink($comment_post_id)) ?>" target="_blank">
                            <?php echo sanitize_text_field(get_the_title($comment_post_id)); ?>
                        </a>
                    <?php }
                    break;
                }
            case 'comment_date' :
                {
                    ...
                    if (in_array(get_post_type($comment_post_id), $lms_types)) {
                        echo sanitize_text_field($comment_date);
                    } else { ?>
                        <a href="<?php echo esc_url(get_comment_link($comment_ID)) ?>" target="_blank">
                            <?php echo sanitize_text_field($comment_date); ?>
                        </a>
                    <?php }
                    break;
                }
    ...
    public static function add_comment()
    {
        ...
        if (empty($_GET['post_id']) or empty($_GET['course_id'])) die;
    ...
    public static function _add_comment($user, $lesson_id, $comment, $parent, $course_id = 0)
    {
        ...
        if (!$r['error']) {
            ...
            $message = sprintf(
                esc_html__('%s commented - "%s" on lesson %s in a course %s', 'masterstudy-lms-learning-management-system'),
                $user_login,
                $comment_content,
                $lesson_title,
				$course_title
