...
class STM_LMS_Course
{
    ...
    public static function is_course_time_expired($user_id, $course_id)
    {
        ...
        if (empty($course['end_time'])) return false;

        if (!empty($course['end_time'])) {
            $course_end_time = $course['end_time'];
            if ($course_end_time - time() < 0) return true;
        }
    ...
    static function course_tabs($tabs, $id)
    {
        ...
        foreach($tabs as $tab_key => $tab_name) {
            if(!STM_LMS_Options::get_option("course_tab_{$tab_key}", 'on')) unset($tabs[$tab_key]);
        }
    ...
    static function get_course_curriculum($course_id) {
    ...
    static function check_course_item($course_id, $item_id) {

		$sections = self::get_course_curriculum($course_id);

		$is_scorm = ($item_id == '0');

		if(empty($sections['curriculum']) && !$is_scorm) STM_LMS_User::js_redirect(get_permalink($course_id));

		if(!in_array($item_id, $sections['curriculum'])  && !$is_scorm) STM_LMS_User::js_redirect( STM_LMS_Lesson::get_lesson_url( $course_id, (self::get_first_lesson($course_id) ) ) );
