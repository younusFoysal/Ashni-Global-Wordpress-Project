...
abstract class LmsUpdates {
	...
	private static $updates = [
		'2.6.0' => ['lms_chat_columns'], // LMS Chat system update with fixes
		'2.6.4' => ['lms_page_routes'], // LMS Chat system update with fixes
		'2.6.7' => ['lms_admin_notification_transient'], // LMS Rate Us Admin Notification
	];
			...
			foreach ( $updates as $version => $callback_arr) {
				if ( version_compare( $current_db_version, $version, '<' ) ) {
					foreach ($callback_arr as $callback) {
						call_user_func( ["\\stmLms\\Classes\\Vendor\\LmsUpdateCallbacks", $callback] );
					}
				}
			}
		}
