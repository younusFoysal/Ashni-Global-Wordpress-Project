...
class PayPal extends StmPaymentGateway{
	...
	public function __construct() {
		...
		$this->supports           = apply_filters('stm_lms_paypal_supports', ['one_time_payment']);
	...
	public static function init() {
		$paypal = new PayPal();
		add_action("stm_lms_payout_settings_save", [$paypal, "save_settings"]);
		add_filter("stm_lms_payout_methods", [$paypal, "add_payment_method"]);
		add_filter("stm_lms_payout_author_fee", [$paypal, "payout_author_fee"]);
	}
	...
	public function save_settings($data) {
		if(isset($_POST['StmPaypal'])) {
			foreach ($_POST['StmPaypal'] as $key => $val){
				$this->update_option(esc_attr($key), esc_attr($val));
			}
			if(!isset($_POST['StmPaypal']['verifying_webhooks']))
	...
	public function render_settings() {
		return stm_lms_render(STM_LMS_PATH . '/libraries/paypal/includes/admin/views/settings/index', ['paypal' => $this]);
	}
	...
	public function payout_author_fee($payouts){
		if( $this->enabled == "no" OR $this->id != get_option("stm_lms_payout_default") OR isset($payouts['success']))
			return $payouts;
		$payout = new Payout($this);
		$result = $payout->create_batch_payout($payouts);
		if($result['success'] AND !empty($result['output'])) {
