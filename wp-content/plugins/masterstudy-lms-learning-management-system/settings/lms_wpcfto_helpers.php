...
class STM_LMS_WPCFTO_HELPERS
{
    ...
    function questions_modify_search($response, $post_type)
    {
    ...
    static function get_question_fields()
    {
        return array(
            'type' => array(
    ...
    function question_fields($post_id)
    {
        ...
        if (!empty($meta['answers']) and !empty($meta['answers'][0] and !empty($meta['answers'][0]['categories']))) {
    ...
    function stm_wpcfto_add_vc_icons($fonts)
    {
    ...
    function add_wpcfto_category_icons($sets)
    {
    ...
    function wpcfto_field_addon_state($classes, $field_name, $field)
    {
    ...
    function is_addon($classes, $field_name, $field) {
        return in_array('is_pro is_pro_in_addon', $classes);
    }

    function addon_state($field_name) {
        $addon_state = apply_filters("wpcfto_addon_option_{$field_name}", '');
    ...
    function start_field($classes, $field_name, $field, $is_pro, $pro_url) {
        ...
        if ($is_pro === 'is_pro'): ?>
                ...
                <span class="pro-notice">
                    <?php _e(
                        'Available in <a href="' . esc_url($pro_url) . '" target="_blank">Pro Version</a>',
