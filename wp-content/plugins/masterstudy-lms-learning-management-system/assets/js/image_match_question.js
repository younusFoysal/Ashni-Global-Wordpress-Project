  ...
  $(document).ready(function () {
    ...
    function init_sortable() {
      $('.stm_lms_question_image_match').each(function () {
        $('.stm_lms_question_image_match__matches, .stm_lms_question_image_match__answer').sortable({
          connectWith: '.stm_lms_question_image_match__answer',
          appendTo: document.body,
          helper: 'clone',
          over: function stop(event, ui) {
            $(ui.placeholder).closest('.stm_lms_question_image_match__answer').removeClass('empty');
          },
          out: function start(event, ui) {
            var parent_item = $(ui.placeholder).closest('.stm_lms_question_image_match__answer');
          ...
          receive: function receive(event, ui) {
            var $parent = $(this);
            var $donor = $(ui.sender);
            $parent.closest('.stm_lms_question_image_match__answer').removeClass('empty');
            setTimeout(function () {
              if ($donor.html().length < 1) {
                $donor.closest('.stm_lms_question_image_match__answer').addClass('empty');
              }
            }, 0);
            ...
            var items = [];
            var $answers = $parent.closest('.stm_lms_question_image_match__questions').find('.stm_lms_question_image_match__answer');

            if ($answers.length < 1) {
              $answers = $parent.closest('.stm_lms_question_image_match__questions').find('.stm_lms_question_image_match__single');
            }
