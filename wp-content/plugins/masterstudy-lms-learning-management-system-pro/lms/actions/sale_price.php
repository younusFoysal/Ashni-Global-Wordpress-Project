...
function stm_lms_get_sale_price_pro($sale_price, $course_id) {
	...
	if(empty(intval($sale_start)) and empty(intval($sale_end))) return $sale_price;

	$now = time() * 1000;
	return ($now > $sale_start and $now < $sale_end) ? $sale_price : 0;
}
...
function stm_lms_sale_price_meta_pro($price, $meta, $default_price) {
    ...
    if(!isset($meta['sale_price_dates_start']) or !isset($meta['sale_price_dates_end'])) return $price;

	if(empty(intval($meta['sale_price_dates_start'])) and empty(intval($meta['sale_price_dates_end']))) return $price;
	...
	return ($now > $sale_start and $now < $sale_end) ? $price : $default_price;
}