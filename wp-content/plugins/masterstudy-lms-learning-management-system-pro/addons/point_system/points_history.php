...
class STM_LMS_Point_History
{
    ...
    static function per_row() {
        return 20;
    }

    static function points($user_id = '') {
        ...
        foreach($points as &$point) {
            $point['timestamp'] = stm_lms_time_elapsed_string(date('Y-m-d H:i:s', $point['timestamp']));
            $point['data'] = $points_info[$point['action_id']];
            if($point['score'] > 0 )$point['score'] = "+{$point['score']}";
    ...
    static function points_history_url() {
        $settings = get_option('stm_lms_settings', array());

        if (empty($settings['user_url']) or !did_action('init')) return home_url('/');

        return get_the_permalink($settings['user_url']) . 'points-history';
    }
