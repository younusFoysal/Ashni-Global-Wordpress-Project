...
wp_enqueue_script('moment.min', STM_LMS_URL . '/assets/vendors/moment.min.js');
wp_enqueue_script('chart.min', STM_LMS_URL . '/assets/vendors/chart.min.js');
wp_enqueue_script('vue-chartjs.min', STM_LMS_URL . '/assets/vendors/vue-chartjs.min.js');
...
stm_lms_register_script('account-statistics', ['jquery', 'moment.min'], false, " Vue.http.options.root = '" . get_site_url() . STM_LMS_BASE_API_URL . "'; var account_statistics_data = JSON.parse('" . stm_lms_convert_content(json_encode($data)) . "');");
...
<div id="stm-account-statistics" v-bind:class="{'min-height-500': order_items.length}">
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-6">
                <div class="stat-actions">
                    ...
                    <button @click="save_email" class="btn btn-default"
    ...
    <div v-if="order_items.length" class="text-center p-b-30">
        <button v-if="show_load_more" @click="load_more()" class="btn btn-default"
    ...
    <button v-if="!order_items.length" @click="date_range = null"
            class="btn btn-default"><?php esc_html_e('Clear Filter', 'masterstudy-lms-learning-management-system-pro'); ?>
    </button>
    <h3 class="p-t-30"
        v-if="!order_items.length"><?php _e('Statistics have not yet appeared.', 'masterstudy-lms-learning-management-system-pro') ?></h3>
</div>
